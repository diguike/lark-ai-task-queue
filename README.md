...
- **按需飞书推送**:**默认无事不打扰** —— 仅当本轮有实质活动(任务完成/等你确认/失败/条件触发)才私聊你一条小结;空轮、"无未读·波动不大·无变化"只写日志。`notify.when` 可选 `on_activity`(默认)/`always`/`off`,渠道 `off`/`bot`/`webhook`。
...
| | `when` | 何时推送:`on_activity`(默认,无实质活动不打扰) / `always`(每轮都发) / `off`(从不) |
...
| **按需推送** | `notify.when=on_activity`(默认)下**只有实质活动才推送**(完成/等确认/失败/条件触发),空轮与"无变化/无未读/波动不大"静默只记日志;任务评论(留痕)始终回写,与推送解耦。`when` 另可 `always`/`off`,`channel` 可选 `off`/`bot`/`webhook`(webhook 走 Node `fetch`)。 |

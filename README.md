...
| 依赖 | 用途 | 安装 |
|---|---|---|
| `lark-cli` | 调飞书任务 / 文档 / 消息 API | 见 [lark-cli 文档](https://github.com/larksuite) 或你团队内部分发渠道 |
| `claude`(Claude Code) | 真正执行任务的 AI 执行器 | `npm i -g @anthropic-ai/claude-code` |
| `jq` | shell 里解析 JSON | macOS `brew install jq` / Debian `apt install jq` |
| `curl`、`bash` | webhook 推送 / 脚本 | 系统自带 |

装好后跑一键体检,逐项告诉你还缺什么:
```bash
bash scripts/doctor.sh     # 体检:依赖 / 认证 / 配置 / 清单发现
```

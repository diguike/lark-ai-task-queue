...
> 封装在 `src/core/lark.mjs` 的 `listComments` 与 `src/core/confirm.mjs` 的 `checkConfirmation`(none|waiting|confirmed,纯逻辑 `evaluateConfirmation` 可单测)。
...
```
封装在 `src/core/notify.mjs` 的 `notify`(off/bot/webhook;webhook 走 Node `fetch`)。
